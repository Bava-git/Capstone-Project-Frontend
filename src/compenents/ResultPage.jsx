...
import selected_sleper from '../assets/icons/sl_selected.svg'
...
const ResultPage = () => {
    ...
    const handleBusLayout = (Bus, schedule) => {
        ...
        let blockedSeats = BusLogs.filter((element) => {
            return element.booked_date === format(schedule.boardingDateTime, "yyyy-MM-dd");
        })
        blockedSeats = blockedSeats.filter((element) => {
        ...
        setLowerLeft(leftseatnum.map((seat, index) => (
            <div className='perseat' key={seat}>
                {((blockedSeats[index]?.bookedSeatNum) === seat) ? (
                    <label className='selector'>
                        <input type="checkbox" disabled
                            checked={selectedArr.includes(seat)}
                            onChange={(event) => handleSelector(seat, event, Bus, schedule)} />
                        {leftseattype === "Seater" ?
                            <img src={(blockedSeats[index]?.passenger_gender) === "Male" ? blocked_male_seater : blocked_female_seater} alt="" className='seater-icon' />
                            : (<img src={(blockedSeats[index]?.passenger_gender) === "Male" ? blocked_male_sleeper : blocked_female_sleeper} alt="" className='sleeper-icon' />)
                        }
                    ...
                    <label className='selector'>
                        ...
                        {leftseattype === "Seater" ?
                            ...
                            : (<img src={selectedArr.includes(seat) ? selected_sleper : available_sleeper} alt="" className='sleeper-icon' />)
                        }
                        {leftseattype === "Seater" ?
        ...
        setLowerRight(rightseatnum.map((seat, index) => (
    ...
    const handleBooking = () => {
        ...
        // passengerStorage.forEach((element) => {
        //     console.log(element);
        // });
